...
# 한쪽 경로만 requirements와 limitation_checks를 빼고 물으면, 같은 청구항이 최초 분석에
# 포함됐는지 나중에 추가됐는지에 따라 다른 결과가 나옵니다.
...
곳에도 걸리지 않습니다. 그 틈으로 장치의 정상 동작 기재가 "…를 획득함" 한정의 근거로
통과하면, 구성 하나가 근거 없이 전부 개시로 보고됩니다.
...
def _chunk_payload(chunk) -> dict:
    ...
    문헌 4건 × 3표본 기준으로 프롬프트의 12% 남짓이 여기입니다.
    ...
    확인해야 합니다(캐시 키가 프롬프트 문면으로 갈리므로 두 버전을 나란히 돌릴 수 있습니다).
    """
...
def compare_document_claims(claims: list[Claim], document: Document, guideline: str = "",
    ...
    묶는 축이 문헌이라 문헌 본문이 프롬프트에 **문헌당 한 번만** 실립니다. (청구항 × 문헌)
    축으로 묶으면 같은 문헌이 청구항 수 × 표본 수만큼 다시 실려, 13만 자짜리 문헌 하나가
    10항 기준 500만 자를 넘깁니다.
...
def batch_budget(document_count: int) -> int:
    ...
    계산이 select_chunks_for_claims 안에만 있으면 키에는 종속항 단건 예산이 적히고
    프롬프트에는 이 값이 실려, 둘이 어긋납니다.
    """
...
def select_chunks_for_claims(claims: list[Claim], document: Document,
    ...
    # 청크는 문헌당 **한 번만** 토큰화합니다. 구성요소마다 전 청크를 다시 훑으면 청크
    # 500개 × 구성 20개에서 정규식 토큰화가 1만 번 돌고, 문헌 수만큼 곱해집니다.
    ...
    used = 0
    # 큐를 소비할 때 리스트를 다시 만들지 않고 커서만 옮깁니다. 꺼낼 때마다 큐 전체를
    # 재작성하면 (구성 × 청크²)로 늘어납니다.
...
def _build_matches(raw_matches, claim: Claim, document: Document
    ...
    하위 제한 점검은 항상 요구합니다. 끌 수 있게 두면 같은 청구항이 최초 분석에 있었는지
    나중에 추가됐는지에 따라 다른 판정을 받습니다.
    """
...
def _response_strength(item: dict) -> tuple:
    ...
    등급은 코드가 산출하므로 응답에 라벨이 없고, **등급을 정하는 것이 근거 있는 개시 한정의
    수**이므로 그것을 첫 기준으로 둡니다. 모델이 문헌 앞쪽 총론으로 한 번 답한 뒤 뒤쪽
    실시예를 찾아 다시 답한 경우 근거가 실린 쪽이 남아야 합니다.
    """

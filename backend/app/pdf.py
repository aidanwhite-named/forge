...
한 문장을 두 동강 내므로, 그대로 두면 발췌 검증이 문헌 전체에서 실패합니다. 낱말 좌표로
단을 갈라 읽기 순서를 복원한 뒤에야 원문 대조가 의미를 갖습니다.
"""
import re
...
# 공보 단락번호. 추출기가 대괄호 안에 공백을 넣는 경우가 많아 공백을 허용합니다.
_PARAGRAPH_RE = re.compile(r"\[\s*(\d{4})\s*\]")
_PARAGRAPH_START_RE = re.compile(r"^\s*\[\s*\d{4}\s*\]")
...
# 2단 조판 판별·복원 상수. 전부 페이지 좌표(pt) 기준입니다.
_GUTTER_MARGIN = 6.0        # 중앙 경계선의 좌우 여유 폭
_MIN_COLUMN_WORDS = 40      # 한 단으로 인정할 최소 낱말 수
_STRADDLE_RATIO = 0.02      # 중앙을 가로지르는 낱말이 이 비율을 넘으면 단일 단으로 봅니다
_LINE_TOLERANCE = 3.0       # 같은 줄로 묶을 y 오차
...
def extract_pdf(path: Path, document_id: str) -> Document:
    ...
    chunks = _chunk_pages(pages, document_id, doc_type)
...
def page_text(page) -> str:
    ...
    복원에 실패할 만한 페이지(낱말이 적은 표지·도면 등)는 추출기 기본 순서를 그대로 씁니다.
    """
    words = page.get_text("words")          # (x0, y0, x1, y1, word, block, line, word_no)
    if len(words) < _MIN_COLUMN_WORDS * 2:
        return page.get_text("text").strip()
    columns = [_column_text(column) for column in _split_columns(words, page.rect)]
    return "\n\n".join(column for column in columns if column).strip()


def _split_columns(words: list, rect) -> list[list]:
    """중앙선을 가로지르는 낱말이 거의 없을 때만 2단으로 봅니다.

    표·수식·단 통합 제목이 많은 페이지는 가로지르는 낱말이 늘어나므로 단일 단으로 남습니다.
    """
    middle = (rect.x0 + rect.x1) / 2
    straddling = sum(1 for word in words
                     if word[0] < middle - _GUTTER_MARGIN and word[2] > middle + _GUTTER_MARGIN)
    if straddling > len(words) * _STRADDLE_RATIO:
        return [words]
    left = [word for word in words if (word[0] + word[2]) / 2 <= middle]
    right = [word for word in words if (word[0] + word[2]) / 2 > middle]
    if min(len(left), len(right)) < _MIN_COLUMN_WORDS:
        return [words]
    return [left, right]


def _column_text(words: list) -> str:
    """단 하나를 줄 단위로 세우고, 줄 간격이 벌어진 곳을 문단 경계로 되살립니다.
    ...
    if not lines:
        return ""
    gaps = [later - earlier for (earlier, _), (later, _) in zip(lines, lines[1:]) if later > earlier]
    typical = median(gaps) if gaps else 0.0
    text = [lines[0][1]]
    for (previous, _), (top, line) in zip(lines, lines[1:]):
        new_paragraph = (typical and top - previous > typical * _PARAGRAPH_GAP) or _PARAGRAPH_START_RE.match(line)
        text.append(("\n\n" if new_paragraph else "\n") + line)
    return "".join(text)


def _group_lines(words: list) -> list[tuple[float, str]]:
    """같은 y에 놓인 낱말을 한 줄로 묶습니다. 줄 안에서는 x 순서를 따릅니다."""
    lines: list[tuple[float, list]] = []
    for word in sorted(words, key=lambda word: (round(word[1], 1), word[0])):
        if lines and abs(word[1] - lines[-1][0]) <= _LINE_TOLERANCE:
            lines[-1][1].append(word)
        else:
            lines.append((word[1], [word]))
    return [(top, " ".join(word[4] for word in sorted(group, key=lambda word: word[0])))
            for top, group in lines]


def classify(text: str) -> str:
    """단락번호와 공보 서지 어휘를 우선 보고, 그 다음 논문 어휘를 봅니다."""
    normalized = text.lower()
    if len(_PARAGRAPH_RE.findall(text)) >= 3:
        return "patent"
...
def _chunk_pages(pages: list[str], document_id: str, doc_type: str) -> list[Chunk]:
    chunks: list[Chunk] = []
    section = ""
    for page_number, page_text in enumerate(pages, 1):
        ...
        for block in re.split(r"\n\s*\n", page_text):
            ...
            if doc_type == "paper" and _is_excluded_section(section):
                continue
            paragraph = _PARAGRAPH_RE.search(text)
            if paragraph:
                chunk_id = f"D{document_id}-P-{paragraph.group(1)}"
            else:
                block_index += 1
